...
void CheatMenu::drawFlightHUD(video::IVideoDriver *driver, const v3f &pos, const v3f &vel, f32 pitch, f32 yaw)
{
	if (!g_settings->getBool("flight_hud"))
		return;

	auto ss = driver->getScreenSize();
	s32 cx = ss.Width - 120;
	s32 cy = ss.Height - 180;
	s32 radius = 50;

	video::SColor white(255, 255, 255, 255);
	video::SColor dim(200, 140, 140, 140);
	video::SColor red(255, 255, 60, 60);

	// ---- Artificial Horizon ----
	// Circle outline
	for (s32 a = 0; a < 32; a++) {
		f32 a1 = a * 2 * core::PI / 32;
		f32 a2 = (a + 1) * 2 * core::PI / 32;
		driver->draw2DLine(core::position2d<s32>(cx + (s32)(radius * cos(a1)), cy + (s32)(radius * sin(a1))),
			core::position2d<s32>(cx + (s32)(radius * cos(a2)), cy + (s32)(radius * sin(a2))), white);
	}

	// Artificial horizon: horizontal line that moves up/down with pitch
	// Pitch down → line moves up; pitch up → line moves down
	f32 pitch_offset = -pitch * (radius / 45.0f); // ±45° maps to ±radius
	s32 horizon_y = cy + (s32)pitch_offset;
	s32 horizon_limit = radius - 6;
	// Clamp horizon line to stay within circle
	s32 line_len = (s32)sqrt((f32)(radius * radius - pitch_offset * pitch_offset)) - 2;
	if (line_len > horizon_limit) line_len = horizon_limit;
	if (line_len > 4) {
		driver->draw2DLine(core::position2d<s32>(cx - line_len, horizon_y),
			core::position2d<s32>(cx + line_len, horizon_y), white);
	}

	// Sky/ground fill above/below horizon
	video::SColor sky_color(80, 40, 60, 180);
	video::SColor ground_color(80, 80, 60, 40);
	driver->draw2DRectangle(sky_color, core::rect<s32>(cx - radius, cy + (s32)pitch_offset - radius,
		cx + radius, cy + (s32)pitch_offset));
	driver->draw2DRectangle(ground_color, core::rect<s32>(cx - radius, cy + (s32)pitch_offset,
		cx + radius, cy + (s32)pitch_offset + radius));

	// Center indicator (fixed marker)
	driver->draw2DLine(core::position2d<s32>(cx - 8, cy), core::position2d<s32>(cx + 8, cy), dim);
	driver->draw2DLine(core::position2d<s32>(cx, cy - 4), core::position2d<s32>(cx, cy + 4), dim);

	// Pitch degree marks (move with horizon)
	for (s32 deg = -20; deg <= 20; deg += 5) {
		if (deg == 0) continue;
		s32 mark_y = cy + (s32)(-deg * (radius / 45.0f));
		s32 mk = (deg % 10 == 0) ? 10 : 5;
		driver->draw2DLine(core::position2d<s32>(cx - mk, mark_y),
			core::position2d<s32>(cx + mk, mark_y), dim);
	}

	// ---- Altimeter (vertical bar) ----
	s32 abar_x = cx + radius + 20;
	s32 abar_w = 16;
	s32 abar_h = radius * 2;
	s32 abar_y = cy - radius;

	// Background
	driver->draw2DRectangle(video::SColor(120, 0, 0, 0), core::rect<s32>(abar_x, abar_y, abar_x + abar_w, abar_y + abar_h));

	// Y position mapped to bar (-32000 to +32000)
	f32 y_norm = (pos.Y + 32000.0f) / 64000.0f;
	y_norm = std::fmax(0.0f, std::fmin(1.0f, y_norm));
	s32 fill_h = (s32)(y_norm * abar_h);
	video::SColor alt_color = video::SColor(200, 60, 180, 60);
	driver->draw2DRectangle(alt_color, core::rect<s32>(abar_x, abar_y + abar_h - fill_h, abar_x + abar_w, abar_y + abar_h));

	// Altitude text
	char atxt[32];
	snprintf(atxt, sizeof(atxt), "Y: %.0f", pos.Y);
	core::dimension2d<u32> adim = m_font->getDimension(utf8_to_wide(atxt).c_str());
	core::rect<s32> afr(abar_x - (s32)adim.Width / 2 + abar_w / 2, abar_y + abar_h + 4,
		abar_x + abar_w / 2 + (s32)adim.Width / 2, abar_y + abar_h + 4 + adim.Height);
	m_font->draw(utf8_to_wide(atxt).c_str(), afr, alt_color, false, false);

	// ---- Tachometer (speed in nodes/sec) ----
	f32 speed_nodes = vel.getLength() / BS;
	s32 tach_x = cx - 60;
	s32 tach_y = cy + radius + 30;

	// Speed bar (horizontal)
	s32 tach_w = 120;
	s32 tach_h = 10;
	driver->draw2DRectangle(video::SColor(120, 0, 0, 0), core::rect<s32>(tach_x, tach_y, tach_x + tach_w, tach_y + tach_h));

	f32 speed_norm = std::fmin(1.0f, speed_nodes / 4.0f);
	video::SColor spd_color = speed_nodes > 2.0f ? red : video::SColor(200, 200, 200, 60);
	driver->draw2DRectangle(spd_color, core::rect<s32>(tach_x, tach_y, tach_x + (s32)(speed_norm * tach_w), tach_y + tach_h));

	// Speed text
	char stxt[32];
	snprintf(stxt, sizeof(stxt), "%.1f n/s", speed_nodes);
	core::dimension2d<u32> sdim = m_font->getDimension(utf8_to_wide(stxt).c_str());
	core::rect<s32> sfr(tach_x + tach_w / 2 - (s32)sdim.Width / 2, tach_y + tach_h + 2,
		tach_x + tach_w / 2 + (s32)sdim.Width / 2, tach_y + tach_h + 2 + sdim.Height);
	m_font->draw(utf8_to_wide(stxt).c_str(), sfr, white, false, false);
}
