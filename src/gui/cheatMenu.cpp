		...
		bool selected, bool active, CheatMenuEntryType entry_type)
{
	...
	if (selected) {
		core::position2d<s32> ul(x - 1, y - 1);
		core::position2d<s32> ur(x + width, y - 1);
		core::position2d<s32> ll(x - 1, y + height);
		core::position2d<s32> lr(x + width, y + height);
		driver->draw2DLine(ul, ur, *fontcolor);
		driver->draw2DLine(ur, lr, *fontcolor);
		driver->draw2DLine(lr, ll, *fontcolor);
		driver->draw2DLine(ll, ul, *fontcolor);
	}
