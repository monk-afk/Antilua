...
function test_wasplib_constraint(T)
	...
	T.run("/cpos1 chat command exists", function()
		T.assert(type(minetest.registered_chatcommands["cpos1"]) == "table")
	end)
	T.run("/cpos2 chat command exists", function()
		T.assert(type(minetest.registered_chatcommands["cpos2"]) == "table")
	end)
	T.run("/creset chat command exists", function()
		T.assert(type(minetest.registered_chatcommands["creset"]) == "table")
	end)
...
function test_wasplib_merged(T)
	...
	T.run("lavaalarm cheat setting exists", function()
...
function test_dig_mod(T)
	...
	T.run("dig/sponge: digcyl chat commands exist", function()
		T.assert(type(minetest.registered_chatcommands["digcyl"]) == "table")
		T.assert(type(minetest.registered_chatcommands["digcyl_rad"]) == "table")
	end)
...
function test_place_mod(T)
	...
	T.run("place/init: BlockWater setting exists", function()
		T.assert(core.settings:get("block_water") ~= nil)
	end)
...
function test_inv_open_mod(T)
	T.run("/craft chat command exists", function()
		T.assert(type(minetest.registered_chatcommands["craft"]) == "table")
	end)
	T.run("/openlist chat command exists", function()
		T.assert(type(minetest.registered_chatcommands["openlist"]) == "table")
	end)
...
function test_autocraft_mod(T)
	T.run("/autocraft chat command exists", function()
		T.assert(type(minetest.registered_chatcommands["autocraft"]) == "table")
	end)
	T.run("/autocraft_list chat command exists", function()
		T.assert(type(minetest.registered_chatcommands["autocraft_list"]) == "table")
	end)
	T.run("/autocraft_clear chat command exists", function()
		T.assert(type(minetest.registered_chatcommands["autocraft_clear"]) == "table")
	end)
	T.run("autocraft cheat setting exists", function()
		T.assert(core.settings:get("autocraft") ~= nil)
	end)
	T.run("autocraft_recipes setting exists", function()
		T.assert(core.settings:get("autocraft_recipes") ~= nil)
	end)
end
...
function test_world_interaction(T)
	T.defer("ws.can_place_at returns bool for current player pos", function()
		...
		if pos then
			...
			local node = minetest.get_node_or_nil(np)
			if node then
...
function test_category_assignments(T)
	...
	T.run("BlockWater is in Place category", function()
		check_category("BlockWater", "Place")
	end)
	T.run("BlockLava is in Place category", function()
		check_category("BlockLava", "Place")
	end)
