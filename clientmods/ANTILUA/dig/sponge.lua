...
ws.rg("DigFreeSponge", {
	...
	on_step = function(self)
		...
		for _, sp in pairs(core.find_nodes_near(lp, range, {"mcl_sponges:sponge", "mcl_sponges:sponge_wet"})) do
			if not core.find_node_near(sp, water_dist, "mcl_core:water_source") then
				ws.dig(sp)
			end
		end
	end,
...
core.register_chatcommand("digcyl", {
	description = "Set dig cylinder center",
	params = "[x,y,z]",
	func = function(p)
		local pos = core.string_to_pos(p)
		if pos then
			digcyl_mid = pos
...
core.register_chatcommand("digcyl_rad", {
...
ws.rg("Digcyl", {
	...
	on_step = function(self)
		...
		local lp = core.localplayer:get_pos()
		for _, v in pairs(core.find_nodes_near(lp, ws.range, nlist.get(nlist.selected), true)) do
			local n = core.get_node_or_nil(v)
