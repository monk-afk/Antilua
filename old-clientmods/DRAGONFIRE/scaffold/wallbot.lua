...
local wall_pos1={x=-1254,y=-30,z=791}
local wall_pos2={x=-1454,y=80,z=983}
local iwall_pos1={x=-1264,y=-30,z=801}
...
local lwltime=0
ws.rg("WallTool","Scaffold","scaffold_walltool", function()
	...
	local i=1
	local nds = {}
	--local nds=minetest.find_nodes_near_except(lp,ws.range,{cobble,'mcl_core:obsidian','mcl_core:stonebrick'})
	--local nds=ws.get_reachable_positions(ws.range)
	for k,vv in pairs(nds) do
		--if i > 8 then return end
		local iwn=iwall_node(vv)
		local nd=minetest.get_node_or_nil(vv)
		if vv and in_wall(vv) and not iwn then
			if nd and nd.name ~= 'air' and nd.name ~= 'mcl_core:water_source' then
				scaffold.dig(vv)
				i = i + 1
			else
				ws.place(vv,{cobble})
...
end,function()end,function()end,{},0.1)
